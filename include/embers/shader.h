//
// Created by Naokitsu on 1/14/2024.
//

#ifndef SHADER_H
#define SHADER_H

#include <set>
#include <stdexcept>
#include <bits/unique_ptr.h>
#include <glad/glad.h>
#include <expected>

namespace embers::shader {

class Shader;

class Error {
  public:
  enum Type {
    Compilation,
    Linking,
    Null,
  };
  explicit Error() noexcept;
  explicit Error(std::unique_ptr<char[]> &&message, Type type) noexcept;
  Error(const Error &error);
  Error(Error &&error);
  [[nodiscard]] Type ErrorType() const noexcept;
  [[nodiscard]] const char *Message() const noexcept;
  ~Error() = default;
  private:
  std::unique_ptr<char[]> message_;
  Type type_;
};

enum Type {
  kVertex = GL_VERTEX_SHADER,
  kFragment = GL_FRAGMENT_SHADER,
};


class Source {
  public:
  Source(const char *source, Type type, size_t length = 0);
  Source(std::istream &input_stream, Type type, size_t length = 0);
  Source(const Source &) = delete;
  [[nodiscard]] std::expected<Shader, Error> Compile() const;
  private:
  std::unique_ptr<char[]> source_;
  Type type_;
};

class Shader {
  public:
  explicit Shader(GLuint shader = 0);
  Shader(const Shader &) = delete;
  ~Shader();
  explicit operator GLuint() const {
    return shader_;
  }

  private:
  GLuint shader_;
};

class Program {
  public:
  class Builder {
    GLint program_;
    std::set<GLuint> attached_shaders_;
    bool is_built_ = false;
    public:
    Builder();
    Builder &AttachShader(Shader shader);
    Builder &DetachShader(Shader shader);
    std::expected<Program, Error> Link();
    ~Builder();
  };

  private:
  GLint program_;
  public:
  explicit Program(GLint program = 0);
  ~Program();

  Program &use();

  Program &setUniform(const char* name, GLboolean value);
  Program &setUniform(const char* name, GLint value);
  Program &setUniform(const char* name, GLfloat value);

  explicit operator GLuint() {
    return program_;
  }
};

}

#endif //SHADER_H
